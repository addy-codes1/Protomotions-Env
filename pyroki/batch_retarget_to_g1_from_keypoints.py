...
def setup_logging(log_path: str) -> logging.Logger:
    ...
    # File handler — always append so restarts accumulate in one log
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to log file (appends on restart). Defaults to <output-dir>/retarget.log",
    )
    ...
    if args.save_contacts_only:
        ...
        log_path = args.log_file or os.path.join(contacts_dir, "contacts.log")
        log = setup_logging(log_path)
        log.info("=" * 60)
        log.info("CONTACTS-ONLY mode — extracting foot contact labels")
        log.info(f"Output dir : {contacts_dir}")
        log.info(f"Log file   : {log_path}")
        log.info(f"Total files: {len(test_keypoints_paths)}")
        log.info("=" * 60)
        ...
        for i, motion_path in enumerate(test_keypoints_paths):
            ...
            try:
                ...
                processed = n_done + n_failed
                log.info(
                    f"[{i+1}/{len(test_keypoints_paths)}] OK  {base_filename}  "
                    f"({elapsed_this:.1f}s)  |  done={n_done}  failed={n_failed}  "
                    f"skipped={n_skipped}  ETA={_eta(total_elapsed, processed, len(test_keypoints_paths) - n_skipped)}"
                )
            except Exception as exc:
                n_failed += 1
                log.error(
                    f"[{i+1}/{len(test_keypoints_paths)}] FAILED  {base_filename}  — {exc}"
                )
                log.debug(traceback.format_exc())

        total_elapsed = time.time() - t_loop_start
        log.info("=" * 60)
        log.info(f"CONTACTS DONE in {datetime.timedelta(seconds=int(total_elapsed))}")
        log.info(f"  Saved  : {n_done}")
    ...
    if args.visualize:
        ...
        log_path = args.log_file or os.path.join(output_dir, "retarget.log")
        log = setup_logging(log_path)

        # Count already-done files up front so ETA is accurate from the start
        n_already_done = 0
        if args.skip_existing:
            for mp in test_keypoints_paths:
                bn = os.path.splitext(os.path.basename(mp))[0]
                if os.path.exists(os.path.join(output_dir, f"{bn}_retargeted.npz")):
                    n_already_done += 1

        n_to_process = len(test_keypoints_paths) - n_already_done

        log.info("=" * 60)
        log.info("RETARGETING mode — non-visualize, saving to disk")
        log.info(f"Output dir    : {output_dir}")
        log.info(f"Log file      : {log_path}")
        log.info(f"Total motions : {len(test_keypoints_paths)}")
        log.info(f"Already done  : {n_already_done}  (--skip-existing)")
        log.info(f"To process    : {n_to_process}")
        log.info("=" * 60)
        ...
        n_failed = 0
        t_loop_start = time.time()
        times_per_motion = []   # rolling window for ETA
        ...
        for i, motion_path in enumerate(test_keypoints_paths):
            ...
            try:
                ...
                elapsed_this = time.time() - t0
                times_per_motion.append(elapsed_this)
                # Keep a rolling window of last 50 motions for ETA
                if len(times_per_motion) > 50:
                    times_per_motion.pop(0)
                avg_t = sum(times_per_motion) / len(times_per_motion)
                remaining = n_to_process - n_done - 1
                eta_str = str(datetime.timedelta(seconds=int(avg_t * remaining)))

                n_done += 1
                total_elapsed = time.time() - t_loop_start
                log.info(
                    f"[{i+1}/{len(test_keypoints_paths)}] OK  {base_filename}  "
                    f"({elapsed_this:.1f}s/motion  avg={avg_t:.1f}s)  |  "
                    f"done={n_done}  failed={n_failed}  skipped={n_skipped}  "
                    f"ETA={eta_str}  elapsed={str(datetime.timedelta(seconds=int(total_elapsed)))}"
                )
            ...
            # Periodic checkpoint summary every 100 motions
            if (n_done + n_failed) > 0 and (n_done + n_failed) % 100 == 0:
                total_elapsed = time.time() - t_loop_start
                log.info(
                    f"--- CHECKPOINT: {n_done+n_failed}/{n_to_process} processed  "
                    f"({n_failed} failed)  "
                    f"elapsed={str(datetime.timedelta(seconds=int(total_elapsed)))} ---"
                )

        total_elapsed = time.time() - t_loop_start
        log.info("=" * 60)
        log.info(f"RETARGETING COMPLETE in {str(datetime.timedelta(seconds=int(total_elapsed)))}")
        log.info(f"  Saved  : {n_done}")
        log.info(f"  Skipped: {n_skipped}")
        log.info(f"  Failed : {n_failed}")
